...
if(isset($_POST['ara'])){
	...
	$ara = $_POST['search'];
	header("Location:../../ara?search=$ara");
...
if (isset($_POST['seourlduzenle'])) {

	$urun_seourl=seo($_POST['tarif_seourl']);

	$kaydet=$db->prepare("UPDATE tarif SET
		tarif_seourl =:tarif_seourl

		WHERE tarif_id={$_POST['tarif_id']}");

	$update=$kaydet->execute(array(
		'tarif_seourl'=> $urun_seourl

	));

	if ($update) {

		Header("Location:../production/tarif.php?durum=ok");

	} else {

		Header("Location:../production/tarif.php?durum=no");
	}
...
if ($_GET['tarifsil']=="ok") {
	...
	$sil=$db->prepare("DELETE from tarif where tarif_id=:tarif_id");
	$kontrol=$sil->execute(array(
...
if (isset($_POST['tarifduzenle'])) {
	...
	$update=$kaydet->execute(array(
		...
		'kategori_id' => $_POST['kategori_id'],
		'tarif_lezzet' => $_POST['tarif_lezzet'],
	...
	if ($update) {
		...
		Header("Location:../production/tarif-duzenle.php?durum=ok&tarif_id=$tarif_id");

	} else {

		Header("Location:../production/tarif-duzenle.php?durum=no&tarif_id=$tarif_id");
	}
...
if ($_GET['tarif_onecikar']=="ok") {

	

	
	$duzenle=$db->prepare("UPDATE tarif SET
		
		tarif_onecikar=:tarif_onecikar
		
		WHERE tarif_id={$_GET['tarif_id']}");
	
	$update=$duzenle->execute(array(


		'tarif_onecikar' => $_GET['tarif_one']
	));
